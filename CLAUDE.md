...
- **`NEXT_PUBLIC_*` values are inlined at *build* time, not runtime.** Railway exposes service variables during the Docker build, but RUN steps only see them via explicit `ARG`. Without `ARG NEXT_PUBLIC_API_URL` + `ENV NEXT_PUBLIC_API_URL=$NEXT_PUBLIC_API_URL` in the web builder stage, the bundled client JS hardcodes the dev fallback (`http://localhost:4000`) and the production browser hits localhost. Symptom: status page stuck on "Loading probes…" with a pending request to localhost in DevTools.
